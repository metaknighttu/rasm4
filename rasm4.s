...
@---------------------------------------------------
	...
	mov	r8, r0			@move search string in r0 to r4
	@ mov	r5, r1		@move string in r1 to r5
	
	bl	string_length		@check length of string 
	
	mov	r6, r0			@move length of string 1 to r6
	sub	r6, #1			@ subtract one
	@mov	r0, r5			@move string 2 to r0



@ searchStart: /* find starting point for substring */
...
findStart:
	...
	mov	r5, r4			@ copy address to r5
 	...
 	cmp	r5, #0			@ if next->NULL
 	beq	endSearch			@	then we're finished

	add	r5, #4			@ shift address to data section
	mov	r9, r5			@ provide address of string to putstring


	mov r0, r5			@ move nodeString to r0
	bl	string_length	@ get length
	mov	r7, r0			@ move length to r7

	mov	r2,	#0			@ nodeString count
	mov	r3, #0			@set count variable


searchLoop:
	ldrb	r0, [r8, r3]		@ load one byte from searchString
	ldrb	r1, [r9, r2]		@load one byte from nodeString
@---------------------------------------------------------------	
	@TOUPPPER
	
	cmp	r0, #96			@check if lower
	subgt	r0, #32			@convert to Upper
	
	cmp	r1, #96			@check if lower
	subgt	r1, #32			@convert to upper
	
@---------------------------------------------------------------
	/* if not equal and if not at end of node */
	cmp	r0, r1			@compare one bit from each string
	bne	loadNext		@ go to next position

	cmp	r3, r6			@compare count to string length
	beq	searchEqual		@break to equal
...
searchEqual:
	mov r0, r5			@ move string address to r0
	bl	putstring		@ print

 	ldr	r5, [r4]		@ dereference next pointer

	/* print and go to next node */
	bl	findStart		@end if at last node
	
loadNext:
	add	r2,	#1			@ increment nodeString
	mov	r3, #0
	cmp	r2, r7			@ check length
	beq findStart		@ next node
	b	searchLoop		@ next byte
@----------------------------------------
endSearch:	
	ldr	r0, =ascBuf		@ load dummy
	mov	r1, #KBSIZE		@ kbsize
